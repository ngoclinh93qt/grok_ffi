...
fn encoder(){
    ...
    //  let image = _grk_image_new(numComps as u16, &mut cmptparm, color_space,true);
     //   let mut imgx = *image;
   ...
   //   let codec = _grk_compress_create(GRK_CODEC_JP2, stream);
     //  _grk_compress_init(codec, &mut params, image);
     //  let mut bSuccess = _grk_compress_start(codec);

    //  if( !bSuccess ) {
          //   _grk_object_unref(stream);
          //   _grk_object_unref(codec);
          //   _grk_object_unref(&mut (*image).obj);
    //    }
     //    bSuccess = _grk_compress(codec);
     //    bSuccess = _grk_compress_end(codec);
